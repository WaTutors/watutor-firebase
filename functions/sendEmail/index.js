 ...
 */
function sendEmail({
  toAddress, html, displayName, subject, res,
}) {
  // send email
  const mailOptions = {
...
exports.testMail = (req, res) => { // for testing use https
  console.log(gmailEmail, gmailPassword, config());
  const { toAddress, displayName } = req.query;
  const subject = 'Welcome to Watutors! (test)';

  if (!toAddress || !displayName) return res.status(400).send('Missing url query params');

  // parse email from template
  let html = welcome; // init template
  html = html.toString();
  ...
  // send email
  return sendEmail({
    toAddress, displayName, subject, html, res,
  });
};
